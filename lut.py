import os
import sys
from tkinter import Tk, Canvas, Button, Label, messagebox, Scale
...
def create_lut():
    ...
    def save_lut():
        ...
        # Create a new image with opacity for the custom LUT
        lut_with_opacity = Image.new("RGBA", (width, height))
        lut_with_opacity = Image.blend(lut_with_opacity, lut, alpha=opacity)

        # Apply hue transformation
        lut_with_opacity = apply_hue_transform(lut_with_opacity, hue)
    ...
    def apply_hue_transform(image, hue_value):
        hsv_image = image.convert("HSV")
        hue_data = hsv_image.split()[0]
        hue_data = hue_data.point(lambda p: (p + hue_value) % 256)
        hsv_image = Image.merge("HSV", (hue_data, hsv_image.split()[1], hsv_image.split()[2]))
        return hsv_image.convert("RGBA")
    ...
    opacity_label = Label(root, text="Opacity", fg="white", bg="#333333")
    opacity_label.place(x=100, y=180)
    opacity_slider = Scale(root, from_=0, to=100, orient="horizontal", length=300, command=update_opacity)
    opacity_slider.set(opacity * 100)
    opacity_slider.place(x=200, y=180)

    # Create the hue slider
    hue_label = Label(root, text="Hue", fg="white", bg="#333333")
    hue_label.place(x=125, y=240)
    hue_slider = Scale(root, from_=0, to=255, orient="horizontal", length=300, command=update_hue)
    hue_slider.set(hue)
    hue_slider.place(x=200, y=240)
